...
contract TokenSale is Ownable {
    ...
    constructor(
        ...
        require(
            ...
            "TokenSale: token address is the zero address"
        );
    ...
    function buyTokens(
        ...
        require(
            _purchasedTokens[beneficiary].add(tokens) <= 50000 ether,
            "TokenSale: exceeding maximum purchase amount"
        );
        require(_weiRaised.add(usdtAmount) <= _cap, "TokenSale: cap exceeded");

        _weiRaised = _weiRaised.add(usdtAmount);
        _purchasedTokens[beneficiary] = _purchasedTokens[beneficiary].add(
            tokens
        ); //update the amount of tokens purchase by the buyer
    ...
    function setEndTime(uint256 newEndTime) external onlyOwner {
        require(
            ...
            "TokenSale: end time must be in the future"
        );
        _endTime = newEndTime;
    }
